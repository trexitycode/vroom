...
```
{
  ...
  "vehicles": [
    { "id": 101, "start_index": 0, "costs": { "per_hour": 3600, "per_km": 0 }, "steps": [
      { "type": "start" },
      { "type": "pickup", "id": 9001 },
  ...
  "shipments": [
    { "amount": [1], "pinned": true, "budget": 1200,
      "pickup": { "id": 9001, "location_index": 1 },
      "delivery": { "id": 9002, "location_index": 2 }
    }
  ],
  "jobs": [ { "id": 3, "location_index": 3, "budget": 700 } ],
...
| [`vehicle_penalties`] | object mapping vehicle ids to signed integer penalty values in user `cost` units. These values are added to the optimization objective when this job is assigned to the given vehicle. Negative values bias assignment *toward* a vehicle; positive values bias *away*. This is a soft preference only: feasibility is unchanged. For shipments, this penalty is applied once for the shipment (counted on the pickup only). Penalties do **not** contribute to route-level budget feasibility checks. |
...
| [`vehicle_penalties`] | object mapping vehicle ids to signed integer penalty values in user `cost` units. Applied once for the whole shipment (counted on pickup only). Negative values bias assignment toward a vehicle; positive values bias away. Soft preference only; feasibility unchanged; does not contribute to route-level budget feasibility checks. |
